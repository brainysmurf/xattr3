...
static PyObject* 
py_getxattr(PyObject* self __attribute__((__unused__)), PyObject *args)
{
    char *path;
    char *name;
    int options = 0;
    ...
    Py_BEGIN_ALLOW_THREADS
    res = xattr_getxattr((const char *)path, (const char *)name, buffer, size, position, options);
    Py_END_ALLOW_THREADS
    if (res == -1) {
        PyObject *tmp = xattr_error_with_filename(path);
        Py_DECREF(buffer);
    ...
    this_result = Py_BuildValue("s#", buffer, size);
    return this_result;
}
...
py_fgetxattr(PyObject* self __attribute__((__unused__)), PyObject *args) /* , PyObject *kwds) */
{
    ...
    char buffer[size];
    Py_BEGIN_ALLOW_THREADS
    size = xattr_fgetxattr(fd, (const char *)name, buffer, size, position, options);
    Py_END_ALLOW_THREADS
    PyMem_Free(name);
    if (size == -1) {
        Py_DECREF(buffer);
...
py_listxattr(PyObject* self __attribute__((__unused__)), PyObject *args) /* , PyObject *kwds) */
{
    ...
    Py_BEGIN_ALLOW_THREADS
    size = xattr_listxattr((const char *)path, NULL, 0, options);
    Py_END_ALLOW_THREADS
    if (size == -1) {    
        PyObject *tmp = xattr_error_with_filename(path);
        PyMem_Free(path);
    ...
    char buffer[size];
    Py_BEGIN_ALLOW_THREADS
    res = xattr_listxattr((const char *)path, buffer, size, options);
    Py_END_ALLOW_THREADS
    if (res == -1) {
        PyObject *tmp = xattr_error_with_filename(path);
        PyMem_Free(path);
    ...
    PyMem_Free(path);
    PyObject *result_this;
    result_this = Py_BuildValue("s#", buffer, size);
    return result_this;
}
...
py_flistxattr(PyObject* self __attribute__((__unused__)), PyObject *args) /* , PyObject *kwds) */
{
    ...
    char buffer[size];
    Py_BEGIN_ALLOW_THREADS
    res = xattr_flistxattr(fd, buffer, size, options);
    Py_END_ALLOW_THREADS
    if (res == -1) {
        Py_DECREF(buffer);
        return xattr_error();
...
static PyMethodDef front_app_methods[] = {
    DEFN(getxattr),
    DEFN(fgetxattr),
    DEFN(setxattr),
    DEFN(fsetxattr),
...
PyDoc_STRVAR(pydoc_moduledoc,
             ""
             );

static PyModuleDef xattr_module[] = {
    (PyModuleDef_Base)PyModuleDef_HEAD_INIT,
    (char *)"front_app",
    pydoc_moduledoc,
    -1,
    front_app_methods,
...
PyMODINIT_FUNC PyInit_front_app(void);

PyMODINIT_FUNC 
PyInit__xattr(void)
{
    PyObject *m;
    m = PyModule_Create(xattr_module);
    return m;
}
